...
CREATE TABLE IF NOT EXISTS settlement_closures (
  ...
  actual_settlement_date          TEXT    NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_settlement_closures_session
  ON settlement_closures (review_session_id);
CREATE INDEX IF NOT EXISTS idx_settlement_closures_deposit
...
BEGIN
  ...
  --     and its post_idempotency_key must equal the closure idempotency_key.
  ...
  -- (2) Deposit must be in the exact closure-eligible state.
  ...
  -- (3) Confirming member must be active OWNER.
  ...
  -- (4) Source (deposit's account) and destination (settlement_account_id)
  ...
  -- (5) Integer reconciliation: received_total = principal + gross_interest
  ...
  -- (6) Document must exist and be SETTLEMENT_EVIDENCE.
